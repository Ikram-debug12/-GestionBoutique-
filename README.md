# Gestion Boutique

Application de gestion de boutique développée en Java Swing avec MySQL.

---

## 📋 Fonctionnalités

- 🔐 **Authentification** (login/mot de passe)
- 📦 **Gestion des produits** (CRUD)
- 👥 **Gestion des clients** (CRUD)
- 💳 **Gestion des ventes** (CRUD)
- 🔍 **Recherche et filtrage** par catégorie et période
- 📊 **Statistiques** avec graphiques JFreeChart

---

## 🛠️ Technologies utilisées

- Java Swing
- MySQL
- JDBC
- JFreeChart
- Inno Setup (installateur Windows)

---

## 📦 Installation

1. Télécharge l'installateur : [`GestionBoutique_Setup.exe`](installateur/GestionBoutique_Setup.exe)
2. Exécute le fichier téléchargé
3. Suis les instructions de l'assistant
4. **Une fois installé, va dans :**  
   `C:\Program Files (x86)\Gestion Boutique\`
5. **Double-clique sur `lancer.bat`** (c'est le lanceur qui fonctionne à coup sûr)

✅ Le fichier `lancer.bat` est fourni et évite toutes les erreurs classiques.


### Identifiants de connexion
- **Login** : `admin`
- **Mot de passe** : `admin`

---

## 🎥 Démonstration vidéo

https://github.com/user-attachments/assets/eb4adef3-42f3-4757-990b-b5dab2a38e37

---

## 📊 Diagrammes UML
 # Diagramme Use Case
 <img width="965" height="608" alt="image" src="https://github.com/user-attachments/assets/a5a96bac-e01d-4689-a26b-34e1d5eb8d68" />
 
  
 # Diagramme de classe 
 <img width="730" height="542" alt="image" src="https://github.com/user-attachments/assets/9eb4d293-72cd-4c58-974d-6173d5717dad" />
 

---

## 🗃️ Base de données

Le fichier SQL est disponible ici :  
[`boutique_db.sql`](dist/boutique_db.sql)

---

## 📝 Licence

Projet réalisé dans le cadre d'un travail universitaire

---

## 👥 Équipe projet

Ce projet a été réalisé en binôme par :

- **Ikram Ait Mazouz** (moi) - [@Ikram-debug12](https://github.com/Ikram-debug12)
- **Fatima Ezzohra Ait Bella** [@zfatime046-debug](https://github.com/zfatime046-debug)
